<?php
/* 
Plugin Name: Agile CRM Email Marketing
Plugin URI: #
Version: 1.0
Author: Agile CRM
Developer: Agile CRM
...
add_action('wp_enqueue_styles', 'agilecrm_email_marketing_css' );
/**
 * Register style sheet.
 */
function agilecrm_email_marketing_css() {
   wp_enqueue_style( 'agilecrm-email-marketing', plugins_url( '/css/style.css', __FILE__ ) );
}
add_action( 'wp', 'agilecrm_email_marketing_landing_page_setpup', 10, 0 ); 
function agilecrm_email_marketing_landing_page_setpup(){
    if(!is_admin()){
        global $post;
        $email_marketing_landing_page = get_post_meta($post->ID, 'email_marketing_landing_page',true);
        if($email_marketing_landing_page != ""){
            $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
            $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
            $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
            if($domain != "" && $email != "" && $rest_api != ""){
               $request  = wp_remote_get('https://'.$domain.'.agilecrm.com/landing/'.$email_marketing_landing_page);
...
add_action( 'init', 'agilecrm_email_marketing_list_agile_form', 10, 0 ); 
function agilecrm_email_marketing_list_agile_form(){
...
add_action('admin_menu', 'agilecrm_email_marketing_create_menu');
function agilecrm_email_marketing_create_menu() {
     add_menu_page('Agile CRM Email Marketing', 'Agile CRM Email Marketing', 'administrator', 'agilecrm_email_marketing_dashboard', 'agilecrm_email_marketing_dashboard_page',plugins_url( 'agile-crm-email-marketing/images/icon.png' ),0);
    add_submenu_page('agilecrm_email_marketing_dashboard', 'Home', 'Home', 'administrator', 'agilecrm_email_marketing_dashboard', 'agilecrm_email_marketing_dashboard_page');
    add_submenu_page('agilecrm_email_marketing_dashboard','Web Rules','Web Rules','administrator', 'agilecrm_email_marketing_webrules', 'agilecrm_email_marketing_webrules_page' );
    add_submenu_page('agilecrm_email_marketing_dashboard','Form Builder','Form Builder','administrator', 'agilecrm_email_marketing_formbuilder', 'agilecrm_email_marketing_formbuilder_page' );
    add_submenu_page('agilecrm_email_marketing_dashboard','Landing Pages','Landing Pages','administrator', 'agilecrm_email_marketing_landing', 'agilecrm_email_marketing_landing_page' );
    add_submenu_page('agilecrm_email_marketing_dashboard','Email Campaigns','Email Campaigns','administrator', 'agilecrm_email_marketing_email', 'agilecrm_email_marketing_email_page' );
    add_submenu_page('agilecrm_email_marketing_dashboard','Web Stats','Web Stats','administrator', 'agilecrm_email_marketing_webstats', 'agilecrm_email_marketing_webstats_page' );
    add_submenu_page('agilecrm_email_marketing_dashboard','Refer a Friend','Refer a Friend','administrator', 'agilecrm_email_marketing_refer', 'agilecrm_email_marketing_refer_page' );
    add_submenu_page('agilecrm_email_marketing_dashboard','Settings','Settings','administrator', 'agilecrm_email_marketing_settings', 'agilecrm_email_marketing_settings_page' );
}
function agilecrm_email_marketing_header($domain){
 $email_marketing_agile_email = (sanitize_email(get_option( "email_marketing_agile_email" )));
 $email_marketing_agile_password = (sanitize_text_field(get_option( "email_marketing_agile_password" )));
 agilecrm_email_marketing_css();
 $page_name = $_GET['page'];
 if($page_name == 'agilecrm_email_marketing_dashboard')
 {
  agilecrm_email_marketing_hidedata_js(); ?> <div id="agilewrapper" class="textaligncenter">
<?php echo "<img src='".plugins_url( '/images/agile-crm.png', __FILE__ )."'  title='Agile Crm logo'/>"; ?> </div>
<?php $webstats= get_option( "email_marketing_agile_webstats" );
    if($webstats === false){
        add_option( "email_marketing_agile_webstats", 1, $deprecated, $autoload );
        $webstats= 1;
    }
    $webrules = get_option( "email_marketing_agile_webrules" );
    if($webrules === false){
        add_option( "email_marketing_agile_webrules", 1, $deprecated, $autoload );
       $webrules = 1;
    }
    $status = $_GET['status'];
    if($status){
...
function agilecrm_email_marketing_dashboard_page(){
    $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
    $password= (sanitize_text_field(get_option( "email_marketing_agile_password" )));
   if(empty($rest_api) || empty($js_api) || empty($domain) || empty($email)){
        agilecrm_email_marketing_settings_page();
    } else {
    $deprecated = null;
    $autoload = 'no';
    if($_POST["featuresform"] && check_admin_referer( 'agilecrm_email_marketing_nonce_action', 'agilecrm_email_marketing_nonce_field' )){
        $webrules = intval($_POST["webrules"]);
        $webstats = intval($_POST["webstats"]);
        if ( get_option( "email_marketing_agile_webrules" ) !== false ) {
            update_option( "email_marketing_agile_webrules", $webrules);
        } else {
            add_option( "email_marketing_agile_webrules", $webrules, $deprecated, $autoload );
        }
             $pages = get_pages(); 
             foreach ( $pages as $page ) {
                 if($webrules == "0")
                     update_post_meta($page->ID , 'email_marketing_enable_webrule', 2);
                 else
                     update_post_meta($page->ID , 'email_marketing_enable_webrule', 1);
             }
        if ( get_option( "email_marketing_agile_webstats" ) !== false ) {
            update_option( "email_marketing_agile_webstats", $webstats);
        } else {
            add_option( "email_marketing_agile_webstats", $webstats, $deprecated, $autoload );
        }
             $pages = get_pages(); 
             foreach ( $pages as $page ) {
                 if($webstats == "0")
                     update_post_meta($page->ID , 'email_marketing_enable_webstats', 2);
                 else
                    update_post_meta($page->ID , 'email_marketing_enable_webstats', 1);
             }
    }
agilecrm_email_marketing_header($domain);
?> <div id="features">
<?php $webrules = get_option( "email_marketing_agile_webrules" );
$webstats = get_option( "email_marketing_agile_webstats" );
?> <form action="#" method="post" >
<?php wp_nonce_field('agilecrm_email_marketing_nonce_action','agilecrm_email_marketing_nonce_field'); ?>
<input type="hidden" name="featuresform" id="featuresform" value="featuresform" />
<a href="admin.php?page=agilecrm_email_marketing_webrules" id="boxm" >
<div class="box">
  <div class="right stripline">
   ...
   <a href="admin.php?page=agilecrm_email_marketing_webrules" class="more">More</a>
  </div>
</div></a>
<a href="admin.php?page=agilecrm_email_marketing_formbuilder" id="boxm">
<div class="box">
  <div class="right stripline">
   ...
   <a href="admin.php?page=agilecrm_email_marketing_formbuilder" class="more">More</a>
   </div> 
 </div></a>
<a href="admin.php?page=agilecrm_email_marketing_landing" id="boxm"> 
   <div class="box">
   <div class="right stripline">
   ...
   <a href="admin.php?page=agilecrm_email_marketing_landing" class="more">More</a>
 </div>
</div> </a>
<a href="admin.php?page=agilecrm_email_marketing_email" id="boxm">
  <div class="box">
   <div class="right stripline">
    ...
    <a href="admin.php?page=agilecrm_email_marketing_email" class="more">More</a>
   </div>
</div> </a>
<a href="admin.php?page=agilecrm_email_marketing_webstats" id="boxm">
<div class="box">
  <div class="right stripline">
    ...
    <a href="admin.php?page=agilecrm_email_marketing_webstats" class="more">More</a>
  </div> 
</div></a> 
<a href="admin.php?page=agilecrm_email_marketing_refer" id="boxm">
  <div class="box">
    <div class="right stripline">
    ...
    <a class="more"  href="admin.php?page=agilecrm_email_marketing_refer">Refer</a>
   </div>
</div></a>
</form>
</div>
...
<?php 
agilecrm_email_marketing_customfeautre_js();
agilecrm_email_marketing_customsubmit_js();
 }
}
function agilecrm_email_marketing_webstats_page(){
     $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
    ...
    if(isset($_POST["featuresform"]) && check_admin_referer( 'agilecrm_email_marketing_nonce_action', 'agilecrm_email_marketing_nonce_field' )){
        $webstats= intval($_POST["webstats"]);
        if ( get_option( "email_marketing_agile_webstats" ) !== false ) {
            update_option( "email_marketing_agile_webstats", $webstats);
        } else {
...
<div class="clear"></div>
<?php agilecrm_email_marketing_customfeautre_js(); ?>
</div>
<?php }
}
function agilecrm_email_marketing_webrules_page(){
    $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
    ...
    $email_marketing_agile_email = (sanitize_email(get_option( "email_marketing_agile_email" )));
    $email_marketing_agile_password = (sanitize_text_field(get_option( "email_marketing_agile_password" )));
    agilecrm_email_marketing_custom_pubnub_refresh_js ();
?><div id="features">
<div class="mainLeftbox">
...
function agilecrm_email_marketing_landing_page(){
    $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
...
function agilecrm_email_marketing_formbuilder_page(){
    $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
...
function agilecrm_email_marketing_refer_page(){
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    agilecrm_email_marketing_header($domain); ?>
<div id="features">
<div class="mainLeftbox">
...
<?php }
function agilecrm_email_marketing_email_page(){
   $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain = (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $email= (sanitize_email(get_option( "email_marketing_agile_email" )));
...
function agilecrm_email_marketing_settings_page(){
    $deprecated = null;
    $autoload = 'no';
   echo $getdoamin = sanitize_text_field($_GET["domain"]);
    ...
    if($getdoamin){
        ...
        $js_api = $js_api_key;
        if($rest_api){
        if ( get_option( "email_marketing_agile_rest_api" ) !== false ) {
            update_option( "email_marketing_agile_rest_api", $rest_api);
        } else {
            add_option( "email_marketing_agile_rest_api", $rest_api, $deprecated, $autoload );
        }

        if ( get_option( "email_marketing_agile_js_api" ) !== false ) {
            update_option( "email_marketing_agile_js_api", $js_api);
        } else {
            add_option( "email_marketing_agile_js_api", $js_api, $deprecated, $autoload );
        }
        if ( get_option( "email_marketing_agile_domain" ) !== false ) {
            update_option( "email_marketing_agile_domain", $domain);
        } else {
            add_option( "email_marketing_agile_domain", $domain, $deprecated, $autoload );
        }

        if ( get_option( "email_marketing_agile_email" ) !== false ) {
            update_option( "email_marketing_agile_email", $email);
        } else {
            add_option( "email_marketing_agile_email", $email, $deprecated, $autoload );
        }
        $hash_password = wp_hash_password( $password );
        if ( get_option( "email_marketing_agile_password" ) !== false ) {
            update_option( "email_marketing_agile_password", $hash_password);
        } else {
            add_option( "email_marketing_agile_password", $hash_password, $deprecated, $autoload );
        } 
    ...
    if($_POST["save"] && check_admin_referer( 'agilecrm_email_marketing_nonce_action', 'agilecrm_email_marketing_nonce_field' )){
    ...
    <div class="well">
    <form action="?page=agilecrm_email_marketing_settings" method="post" >
...
<?php 
//agilecrm_email_marketing_custom_js();
if($email == '') { ?>
...
<?php }
add_action( 'load-post.php', 'agilecrm_email_marketing_page_post_meta_boxes_setup' );
add_action( 'load-post-new.php', 'agilecrm_email_marketing_page_add_post_meta_boxes' );
add_action( 'save_post', 'agilecrm_email_marketing_page_save_postdata' );

function agilecrm_email_marketing_page_post_meta_boxes_setup() {
    add_action( 'add_meta_boxes', 'agilecrm_email_marketing_page_add_post_meta_boxes' );
}

function agilecrm_email_marketing_page_add_post_meta_boxes() {
    add_meta_box('page_section',__('Agilecrm Section', 'page_metabox' ),'agilecrm_email_marketing_page_post_box','page','advanced','high');
}

function agilecrm_email_marketing_page_save_postdata( $post_id ) {
 
    if(isset($_POST["email_marketing_enable_webrule"])){
        $email_marketing_enable_webrule  = intval($_POST["email_marketing_enable_webrule"]);
        update_post_meta($post_id , 'email_marketing_enable_webrule', $email_marketing_enable_webrule);
    }

    if(isset($_POST["email_marketing_enable_webstats"])){
        $email_marketing_enable_webstats = intval($_POST["email_marketing_enable_webstats"]);
        update_post_meta($post_id , 'email_marketing_enable_webstats', $email_marketing_enable_webstats);
    }

    if(isset($_POST["email_marketing_landing_page"])){
        $email_marketing_landing_page = sanitize_text_field($_POST["email_marketing_landing_page"]);
        update_post_meta($post_id , 'email_marketing_landing_page', $email_marketing_landing_page);
    }
}

function agilecrm_email_marketing_page_post_box( $post ) {
    echo "<style>";
    echo "#page-list label{width:50%;float:left}";
    echo "</style>";
    echo "<ul id='page-list'>";
        echo "<li>";
        echo '<label for="email_marketing_enable_webrule">'.__("Enable Web Rule :");
        echo '</label> ';
        $email_marketing_enable_webrule = "";
        if(get_post_meta($post->ID, 'email_marketing_enable_webrule',true) == 1 || get_post_meta($post->ID, 'email_marketing_enable_webrule',true) == "")
            $email_marketing_enable_webrule = " selected " ;

        echo '<select id="email_marketing_enable_webrule" autocomplete="off" name="email_marketing_enable_webrule">';

        echo '<option value="2">No</option>';
        echo '<option value="1" '.$email_marketing_enable_webrule.'>Yes</option>';
        echo '</select>';

        echo "</li>";

        echo "<li>";
        echo '<label for="email_marketing_enable_webstats">'.__("Enable Web Stats :");
        echo '</label> ';
        $email_marketing_enable_webstats = "";

        if(get_post_meta($post->ID, 'email_marketing_enable_webstats',true) == 1 || get_post_meta($post->ID, 'email_marketing_enable_webstats',true) == "")
            $email_marketing_enable_webstats= " selected " ;

 
        echo '<select id="email_marketing_enable_webstats" autocomplete="off" name="email_marketing_enable_webstats">';

        echo '<option value="2">No</option>';
        echo '<option value="1" '.$email_marketing_enable_webstats.'>Yes</option>';
        echo '</select>';
        echo "</li>";
        ...
        if(count($result) > 0){   
        echo "<li>";
        echo '<label for="email_marketing_landing_page">'.__("Landing Page :");
        echo '</label> ';
        $email_marketing_landing_page = get_post_meta($post->ID, 'email_marketing_landing_page',true);
        echo '<select id="email_marketing_landing_page" autocomplete="off" name="email_marketing_landing_page">';
        echo '<option value="">Select</option>';
        foreach($result as $k => $v){
            if($email_marketing_landing_page == $v->id)
        ...
        echo '<label for="email_marketing_landing_page">'.__("Landing Page :");
        echo '</label> ';
        echo '<select id="email_marketing_landing_page" autocomplete="off" name="email_marketing_landing_page" disabled>';
...
add_action('admin_head', 'agilecrm_email_marketing_button');
function agilecrm_email_marketing_button() {
    global $typenow;
    ...
    if ( get_user_option('rich_editing') == 'true') {
        add_filter("mce_external_plugins", "agilecrm_email_marketing_add_tinymce_plugin");
        add_filter('mce_buttons', 'agilecrm_email_marketing_register_button');
    }
}

function agilecrm_email_marketing_add_tinymce_plugin($plugin_array) {
    $plugin_array['agilecrm_email_marketing_button'] = plugins_url( '/js/agile.js', __FILE__ ); // CHANGE THE BUTTON SCRIPT HERE
    return $plugin_array;
}

function agilecrm_email_marketing_register_button($buttons) {
   array_push($buttons, "agilecrm_email_marketing_button");
   return $buttons;
}

add_shortcode('agileform_email_marketing','agileform_email_marketing');
function agileform_email_marketing($atts,$content,$tag){
    if(isset($atts["id"])){
...
function agilecrm_email_marketing_custom_pubnub_refresh_js() {
   $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
     $page = $_GET['page'];
   if($page =='agilecrm_email_marketing_webrules' || $page =='agilecrm_email_marketing_landing' || $page =='agilecrm_email_marketing_formbuilder' || $page =='agilecrm_email_marketing_email'){
        ...
        wp_add_inline_script( 'pubnub_autorefresh', '
          ...
          loadScript("'.plugins_url('/js/pubnub-3.4.min.js', __FILE__).'", function() {
              ...
              Agile_Pubnub.subscribe({ channel : getAgileChannelName(), restore : false, message : function(message, env, channel)
              {
                  ...
                  if(name== "WebRule"){
                    window.location.href = "admin.php?page=agilecrm_email_marketing_webrules";
                  }else if(name== "LandingPages"){
                    window.location.href = "admin.php?page=agilecrm_email_marketing_landing";
                  }else if(name=="Forms"){
                    window.location.href = "admin.php?page=agilecrm_email_marketing_formbuilder";
                  }else if(name=="Campaigns"){
                    window.location.href = "admin.php?page=agilecrm_email_marketing_email";
                  }
              }});  
          });
...
add_action ( 'admin_enqueue_scripts', 'agilecrm_email_marketing_custom_pubnub_refresh_js');


function agilecrm_email_marketing_footer() {
    global $post;
    $js_api = (sanitize_text_field(get_option( "email_marketing_agile_js_api" )));
    $domain= (sanitize_text_field(get_option( "email_marketing_agile_domain" )));
    $rest_api = (sanitize_text_field(get_option( "email_marketing_agile_rest_api" )));
    ...
    $webrules = get_option( "email_marketing_agile_webrules" );
    $webstats = get_option( "email_marketing_agile_webstats" );
    $pagewebrule = get_post_meta($post->ID, 'email_marketing_enable_webrule',true);
    $pagewebstats=get_post_meta($post->ID, 'email_marketing_enable_webstats',true);
    $agile_url = "https://" .$domain. ".agilecrm.com/dev/api/";
    $headers = array(
                'Authorization' => 'Basic ' . base64_encode( $email. ':' .$rest_api ),
                'Content-type' => 'application/json',
...
add_action( 'wp_footer', 'agilecrm_email_marketing_footer');
function agilecrm_email_marketing_customfeautre_js() {
  $page = $_GET['page'];
    if($page != 'agilecrm_email_marketing_settings'){
        wp_enqueue_script('feautre',plugins_url('/js/inputsubmit.js', __FILE__),array('jquery') );
      }
    }
   add_action( 'wp_enqueue_scripts',  'agilecrm_email_marketing_customfeautre_js');
function agilecrm_email_marketing_customsubmit_js() {
        wp_enqueue_script('formsubmit',plugins_url('/js/submitclassform.js', __FILE__),array('jquery') );
    }
   add_action( 'admin_enqueue_scripts',  'agilecrm_email_marketing_customsubmit_js');

function agilecrm_email_marketing_custom_js() {
 ...
 add_action( 'admin_enqueue_scripts',  'agilecrm_email_marketing_custom_js');

function agilecrm_email_marketing_hidedata_js() {
        wp_enqueue_script('hidedata',plugins_url('/js/hidedata.js', __FILE__),array('jquery') );
    }
add_action( 'admin_enqueue_scripts',  'agilecrm_email_marketing_hidedata_js');
/*function agilecrm_email_marketing_customrefresh_js() {
  $page = $_GET['page'];
  $get_domain = $_GET['domain'];
    if($page == 'agilecrm_email_marketing_settings' && $_POST["save"])
    {
          wp_enqueue_script('refresh',plugins_url('/js/refreshpage.js', __FILE__),array('jquery') );
    }
  }
add_action( 'admin_enqueue_scripts',  'agilecrm_email_marketing_customrefresh_js');

function agilecrm_email_marketing_get_customrefresh_js() {
  $page = $_GET['page'];
  $get_domain = $_GET['domain'];
    if($page == 'agilecrm_email_marketing_settings' && $get_domain && $_POST["save"])
    {
          wp_enqueue_script('refresh',plugins_url('/js/refreshpage.js', __FILE__),array('jquery') );
    }
 }
add_action( 'admin_enqueue_scripts',  'agilecrm_email_marketing_get_customrefresh_js');*/
?>